...
export class AuthService {
  ...
  signUp(user: UserRequest) {
    const url = 'http://ec2-13-232-6-4.ap-south-1.compute.amazonaws.com:8080/blogs/users';
  ...
  signIn(user: UserLoginRequestModel) {
    ...
    const url = 'http://ec2-13-232-6-4.ap-south-1.compute.amazonaws.com:8080/blogs/users/login';
  ...
  public getUser(userEmail: string): Observable<boolean> {
    return this.http.get<boolean>(`http://ec2-13-232-6-4.ap-south-1.compute.amazonaws.com:8080/blogs/users/${userEmail}`).pipe(
      map(userData => {
        if (userData) {
          return true;
        } else {
